...
auto Camera::getSensitivity() -> decltype(getSensitivity()) { return sensitivity; }

void Camera::setStep(float _step) { step = _step; }
auto Camera::getStep() -> decltype(getStep()) { return step; }

void Camera::setYaw(float _yaw) { yaw = _yaw; }
auto Camera::getYaw() -> decltype(getYaw()) { return yaw; }

void Camera::setPitch(float _pitch) { pitch = _pitch; }
auto Camera::getPitch() -> decltype(getPitch()) { return pitch; }
...
auto Camera::getTransform() -> decltype(getTransform()) { return transform; }
...
void Camera::rotate(const glm::vec2 &_offset) {
    ...
    if (unclip) {
        ...
        else if (pitch <= -360.f) pitch += 360.f;
    } else {
